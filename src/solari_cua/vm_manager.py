"""Solari MicroVM SDK Integration & Snapshot-Fork Orchestrator.

Implements Task 1.1:
- Lifecycle manager connecting the CUA orchestrator to Solari's Firecracker backend.
...
logger = logging.getLogger("solari_cua.vm_manager")
...
class SolariVMManager:
    ...
    def __init__(
        ...
        image_provider: Optional[SolariImageProvider] = None,
        simulate_hardware: bool = False,
    ):
        """Initialize the Solari VM Lifecycle Manager.

        Args:
            runtime_dir: Base directory for isolated sandboxes (default: /tmp/solari_sandboxes).
            ...
            template_name: Solari template name (e.g. 'base', env SOLARI_BASE_TEMPLATE).
        ...
        if runtime_dir is None:
            self.runtime_dir = Path(tempfile.gettempdir()) / "solari_sandboxes"
        else:
        ...
        self.image_provider = image_provider or SolariImageProvider()
        self.template_name = template_name or os.environ.get("SOLARI_BASE_TEMPLATE", "base")
    ...
    def launch(
        ...
        logger.info(f"Launched Solari MicroVM '{vm_id}' in {launch_latency_ms:.2f}ms (overhead: {mem_size_mib}MB)")
        return meta
    ...
    def terminate(self, vm_id: str) -> bool:
        ...
        logger.info(f"Terminated Solari MicroVM '{vm_id}' and reclaimed isolated sockets.")
        return True
