"""Dynamic Solari MicroVM Image and Kernel Provider.
...
- Aligns with Solari Cloud specification (SOLARI_BASE_TEMPLATE, SOLARI_KERNEL_PATH).
"""
...
logger = logging.getLogger("solari_cua.image_provider")

# Standard search paths for Solari MicroVM images in production environments
DEFAULT_SEARCH_PATHS: List[Path] = [
    Path("/usr/share/solari"),
    Path("/var/lib/solari/images"),
    Path("/opt/solari/images"),
    Path.home() / ".solari" / "images",
]
...
class SolariImageProvider:
    ...
    def resolve(
        ...
        2. Environment variables:
           ...
           - SOLARI_ROOTFS_PATH
           - SOLARI_IMAGE_DIR
           - SOLARI_BASE_TEMPLATE (defaults to 'base')
        ...
        elif os.environ.get("SOLARI_KERNEL_PATH"):
            kernel_path = Path(os.environ["SOLARI_KERNEL_PATH"])
        else:
        ...
        elif os.environ.get("SOLARI_ROOTFS_PATH"):
            rootfs_path = Path(os.environ["SOLARI_ROOTFS_PATH"])
        else:
        ...
        if allow_mock:
            mock_dir = Path(tempfile.gettempdir()) / "solari_mock_images" / template
        ...
        raise FileNotFoundError(
            ...
            f"Set SOLARI_KERNEL_PATH and SOLARI_ROOTFS_PATH or place images in {self.search_paths}."
        )

    def _find_in_search_paths(self, candidates: List[str]) -> Optional[Path]:
        """Check all search paths for candidate filenames."""
        # Also check env SOLARI_IMAGE_DIR if set
        dirs = list(self.search_paths)
        env_img_dir = os.environ.get("SOLARI_IMAGE_DIR")
        if env_img_dir:
