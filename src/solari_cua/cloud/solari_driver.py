...
logger = logging.getLogger("solari_cua.cloud.solari_driver")
...
class SolariCloudDriver:
    ...
    def __init__(
        ...
        self.api_key = api_key if api_key is not None else os.getenv("SOLARI_API_KEY", "").strip()
        self.region = region or os.getenv("SOLARI_REGION", "us-east-1").strip()
        self.api_url = (api_url or os.getenv("SOLARI_API_URL", "https://api.getsolari.com")).rstrip("/")
    ...
    def provision_browser(
        ...
        Returns:
            SolariSession descriptor with CDP endpoint and session metadata.
        """
        ...
        if self.is_mock:
            session = SolariSession(
                ...
                vnc_stream=None,
                replay_url=f"https://cloud.solari.ai/replay/{sid}" if record_session else None,
                region=self.region,
                is_mock=True,
                start_time=now,
                metadata={
        ...
        try:
            ...
            logger.info(f"Live Solari browser provisioned: {actual_sid} [region={self.region}]")
            return session

        except Exception as e:
            logger.warning(f"Failed to provision live Solari browser ({e}). Falling back to mock session.")
            session = SolariSession(
                session_id=sid,
                session_type=SessionType.BROWSER,
                status=SessionStatus.RUNNING,
                cdp_endpoint=f"ws://127.0.0.1:9222/devtools/browser/{sid}",
                replay_url=f"https://cloud.solari.ai/replay/{sid}",
                region=self.region,
                is_mock=True,
                start_time=now,
    ...
    def provision_desktop(
        ...
        """Provision an ephemeral Linux desktop MicroVM in Solari Cloud.
        ...
        Returns:
            SolariSession descriptor with VNC stream and session metadata.
        """
        ...
        try:
            ...
            vnc_stream = res_data.get("vnc_stream", f"wss://{self.region}.cloud.solari.ai/vnc/{sid}")
            cdp_endpoint = res_data.get("cdp_endpoint", f"wss://{self.region}.cloud.solari.ai/cdp/{sid}")
            replay_url = res_data.get("replay_url", f"https://cloud.solari.ai/replay/{sid}")
            ...
            self._sessions[sid] = session
            logger.info(f"Live Solari desktop provisioned: {sid} [region={self.region}]")
            return session

        except Exception as e:
            logger.warning(f"Failed to provision live Solari desktop ({e}). Falling back to mock session.")
            session = SolariSession(
                session_id=sid,
                session_type=SessionType.DESKTOP,
                status=SessionStatus.RUNNING,
    ...
    def terminate(self, session_id: str) -> bool:
        ...
        if not self.is_mock and self.api_key:
            # Send DELETE request to Solari API
            ...
            try:
                ...
                logger.warning(f"Error terminating live Solari session {session_id} on remote API: {e}")

        logger.info(f"Terminated Solari session {session_id} (duration={session.compute_time_ms:.2f}ms)")
        return True
